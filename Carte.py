...
from PySide6.QtGui import QPainter, QColor, QPen, QPixmap, QImage, QTransform
...
class MovingPlane:
    def __init__(self, avion, x, y):
        ...
        self.update_velocity_from_cap()
        self.update_avion_cap()

    # Mouvement & gestion du carburant
    def move(self, w, h):
    ...
    def get_color(self):
        ...
        if fuel > 20:
            ...
            color = warning_color if (self.blink // 30) % 2 == 0 else normal_color
        else:
            color = critical_color if (self.blink // 10) % 2 == 0 else warning_color
...
# Classe : Widget principal du jeu
class GameWidget(QWidget):
    ...
    def __init__(self):
        ...
        # Timer de mise à jour du jeu
        self.timer = QTimer(self)
        self.timer.timeout.connect(self.update_game)
        self.timer.start(16)

        # Timer de spawn d’avions
    ...
    def draw_plane_icon(self, painter, plane: MovingPlane):
        ...
        # Cercle autour de l’avion sélectionné
    ...
    def paintEvent(self, event):
        ...
        if not self.background.isNull():
            scaled = self.background.scaled(
                self.rect().size(), Qt.IgnoreAspectRatio, Qt.SmoothTransformation
            )
    ...
    # Contrôles des avions sélectionnés
    ...
    def update_plane_position(self, plane):
        """Redessine uniquement l'avion passé en paramètre pour réactivité immédiate"""
